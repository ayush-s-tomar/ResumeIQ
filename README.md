...
**TL;DR**
- 🐛 **Fixed a real multi-worker scalability bug** — in-memory caching and rate limiting silently break correctness once you run more than one process; now Redis-backed with automatic fallback (see [Architecture Notes](#-architecture-notes)).
- 📊 **Eval'd, not just claimed** — ATS scoring measured against 15 hand-labeled resume/JD pairs, mean absolute error 5.0 points against human judgment, with the full method in [`eval/run_eval.py`](eval/run_eval.py).
...
| Mean absolute error | 5.0 points |
...
- [x] Eval harness measuring ATS score accuracy against human-judged pairs — 15-pair hand-labeled set, 5.0 mean absolute error (see [Evaluation](#-evaluation))
